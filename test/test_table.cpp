...
TEST(HashTableTest, InsertAndFind) {//работает
...
TEST(HashTableTest, Remove) {//работает
	HashTable<string, int> ht(5);
	ht.insert("apple", 10);
	ht.insert("banana", 20);
...
TEST(HashTableTest, BracketOperator) { //реагирует на 84 строку
	HashTable<string, int> ht(5);
	ht.insert("apple", 10);
	ht.insert("banana", 20);
...
TEST(HashTable, Find) { //присваивание
	HashTable<string, int> ht(10);
	ht.insert("apple", 1);
	ht.insert("banana", 2);
	ht.insert("cherry", 3);
...
TEST(HashTable, Remove) {//скобки
	...
	EXPECT_EQ(ht.find("apple"), ht.end());
	EXPECT_EQ(ht["banana"], 2);
	EXPECT_EQ(ht["cherry"], 3);
}
